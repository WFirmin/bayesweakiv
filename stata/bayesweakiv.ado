				...
				real scalar k)
{
	...
	// Store as dataset variables
    st_store(., "Y", delta*nu + rnormal(250,1,0,1))
    st_store(., "X", Z*rnormal(k,1,0,s) + nu)
    st_store(., tokens("Z1 Z2 Z3 Z4 Z5"), Z)   // one variable per instrument
    st_store(., "cons", J(250, 1, 1))
}
